...
alias ByeBye='
  ...
  if [ "$confirm" = "SI" ]; then
    ...
    # Matar VS Code si está abierto (para que no reescriba archivos al cerrarse)
    pkill -f code 2>/dev/null;
    # Matar Timeshift si está corriendo
    pkill -f timeshift 2>/dev/null;
    ...
    # --- GitKraken instalación global ---
    sudo rm -rf /opt/gitkraken 2>/dev/null;
    sudo rm -f /usr/local/bin/gitkraken 2>/dev/null;
    # --- RustRover instalación global ---
    sudo rm -rf /opt/rustrover 2>/dev/null;
    sudo rm -rf /opt/RustRover 2>/dev/null;
    # --- Desinstalar Timeshift ---
    sudo apt remove --purge timeshift -y 2>/dev/null;
    ...
    # ============================================================
    # --- Limpiar Docker (contenedores, imágenes, volúmenes y redes) ---
    ...
    # --- Borrar snapshots de Timeshift ---
    sudo rm -rf /timeshift 2>/dev/null;
    sudo rm -rf /run/timeshift 2>/dev/null;
    sudo rm -rf /etc/timeshift 2>/dev/null;

    # ============================================================
    # FASE 4: DESTRUCCIÓN DEL HOME
    # ============================================================
    # --- Shred de datos sensibles (incluyendo el propio script en .bashrc/.zshrc) ---
    find $USER_HOME/.ssh -type f -exec shred -u {} \; 2>/dev/null;
    shred -u $USER_HOME/.bash_history $USER_HOME/.zsh_history $USER_HOME/.sh_history $USER_HOME/.python_history $USER_HOME/.bashrc $USER_HOME/.bash_aliases $USER_HOME/.zshrc $USER_HOME/.profile 2>/dev/null;
    ...
    # --- Shred MASIVO de toda la raíz del home (incluyendo ocultos, excepto Papelera) ---
    find $USER_HOME -mindepth 1 -type f -not -path "*/\.Trash/*" -exec shred -u {} \; 2>/dev/null;
    ...
    # --- Editores de código (VSCode, Neovim, Vim) ---
    ...
    # --- Caché de fuentes y miniaturas de video ---
